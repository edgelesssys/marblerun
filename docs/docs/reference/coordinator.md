...
This means all endpoints return a JSON object with a `status` field that is either "success", "fail", or "error":
...
All returned values do not change when a package update has been applied.
...
* `userDefined` bool
    ...
    or if it will we uploaded by a user at a later point (true).
