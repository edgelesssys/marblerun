...
func (r *MultiPartyRecovery) GenerateEncryptionKey(recoveryKeys map[string]string) ([]byte, error) {
	var err error
...
func (r *MultiPartyRecovery) multiPartyRecover(secret []byte) (int, []byte, error) {
	...
	// If we haven't reached max count yet, let an user upload another secret
...
func (r *MultiPartyRecovery) recoverKey(secret []byte) (int, []byte, error) {
	...
	if r.store == nil || reflect.ValueOf(r.store).IsNil() {
		r.log.Debug("Store not set, assuming single-party recovery")
		return 0, secret, nil
	}
...
func generateMultiPartyRecoveryKey(recoveryKeys map[string]string, log *zap.Logger) ([]byte, map[string][]byte, map[string]bool, error) {
	...
	combinedRecoveryKey := make([]byte, 32)
