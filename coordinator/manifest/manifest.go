...
func (m Manifest) CheckUpdate(ctx context.Context, currentPackages map[string]quote.PackageProperties) error {
	if len(m.Packages) <= 0 {
	...
	for packageName, singlePackage := range m.Packages {
		...
		if _, ok := currentPackages[packageName]; !ok {
		...
		if currentPackages[packageName].SecurityVersion != nil && *singlePackage.SecurityVersion < *currentPackages[packageName].SecurityVersion {
