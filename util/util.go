...
// SignPKCS1v15 signs the given data using the given RSA private key.
// The signature will be generated over the SHA-256 hash of the data.
func SignPKCS1v15(signer crypto.Signer, toSign []byte) ([]byte, error) {
	hashed := sha256.Sum256(toSign)
	return signer.Sign(rand.Reader, hashed[:], crypto.SHA256)
}

// VerifyPKCS1v15 verifies a signature generated by [SignPKCS1v15] using the given RSA public key.
func VerifyPKCS1v15(pub *rsa.PublicKey, toVerify, signature []byte) error {
	hashed := sha256.Sum256(toVerify)
