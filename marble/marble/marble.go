...
// EdgMarblePrivKey is the env variable used to store the marble's seal key
...
func PreMain(a *Authenticator, main mainFunc) (*x509.Certificate, *rpc.Parameters, error) {
	...
	for key, secretList := range secrets {
		...
		case "SealKey":
			// Expose Sealing Key
			pemSealKey := pem.EncodeToMemory(&pem.Block{Type: "PRIVATE KEY", Bytes: a.sealKey})
			if err := exposeSecret(secretList, pemSealKey); err != nil {
				return nil, nil, err
			}
		default:
